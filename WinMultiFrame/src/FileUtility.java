...
public class FileUtility {
	...
	// that exist within the directory c:\TASKBPART2FILES and returns the text as a
	// string.
	// The parameter 'filename' - is the name of the file WITHOUT any extension e.g.
	// "file1"
	...
	public static String ReadAllTextInFile(String filename) {
		File directory = new File("c:\\CUSTOMERLIST");
		File file;
		if (filename.isEmpty() || filename == null) {
			System.out.println("Could not read file. No file name was provided!");
		} else {
			if (!directory.exists()) {
				System.out.println("The directory c:\\CUSTOMERLIST does not exist");
			} else {
				file = new File(directory.toString() + "\\" + filename + ".txt");
				Path path = Paths.get(file.getPath());
				if (!file.exists()) {
					System.out
							.println("The file " + filename + ".txt does not exist in the directory c:\\CUSTOMERLIST");
				} else {
					try {
	...
	// If the file name already exists then the text is added/appended to the
	// existing file.
	public static void WriteTextToFile(String filename, String text) {
		...
		if (filename.isEmpty() || filename == null) {
			...
			try {
				PrintWriter writer = new PrintWriter(new BufferedWriter(new FileWriter(file.getPath(), false)));
				writer.println(text);
				writer.close();
			} catch (IOException e) {
